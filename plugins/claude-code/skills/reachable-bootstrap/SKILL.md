...
description: "Bootstrap REACHABLE in a Claude Code workspace with reachable: setup, then verify with reachable: doctor."
---

# REACHABLE Bootstrap

Use this skill when the user asks for `reachable: setup`, `reachable: doctor`,
or REACHABLE workspace setup from Claude Code.
...
For `reachable: setup`, run the REACHABLE setup bootstrap for Claude Code and
the current workspace. Setup may configure the local daemon-backed MCP server
named `reachable` after the runtime is verified.
